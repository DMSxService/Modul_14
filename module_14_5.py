...
async def get_buying_list(message):
    for i in range(1, len(all_products) + 1):
        one_product = all_products[i - 1]
        with open(f'pictures/{i}.jpg', "rb") as img:
            await message.answer(f'Название: {one_product[1]} | Описание: {one_product[2]} | Цена: {one_product[3]}',
                                 reply_markup=types.ReplyKeyboardRemove())
...
async def set_username(message, state):
    include = crud_functions2.is_include(message.text)
    while include:
...
async def set_age(message, state):
    ...
    crud_functions2.add_user(user['username'],user['email'],user['age'])
